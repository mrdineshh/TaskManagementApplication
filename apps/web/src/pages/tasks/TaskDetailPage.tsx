  ...
  return (
    <div className="grid grid-cols-3 gap-6">
      <div className="col-span-2 space-y-4">
        ...
        {notice && (
          <div className="rounded-md border border-amber-300 dark:border-amber-700 bg-amber-50 dark:bg-amber-900/30 px-3 py-2 text-sm text-amber-800 dark:text-amber-300">{notice}</div>
        )}
        ...
        <div className="rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5">
          ...
          <h1 className="text-lg font-semibold text-slate-900 dark:text-slate-100">{task.title}</h1>
          {task.description && <p className="mt-2 whitespace-pre-wrap text-sm text-slate-600 dark:text-slate-400">{task.description}</p>}
          ...
          {availableTransitions.length > 0 && (
            <div className="mt-4 flex flex-wrap gap-2 border-t border-slate-100 dark:border-slate-800 pt-4">
              <span className="text-xs font-medium uppercase text-slate-400 dark:text-slate-500">Move to:</span>
              {availableTransitions.map((t: any) => (
                <button
                  key={t.id}
                  onClick={() => handleTransition(t.to_status_id)}
                  disabled={transitionTask.isPending}
                  className="rounded-md border border-slate-300 dark:border-slate-700 px-2 py-1 text-xs font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 disabled:opacity-50"
                >
          ...
          {pendingHoldStatusId && (
            <div className="mt-3 flex flex-wrap items-center gap-2 rounded-md border border-purple-200 dark:border-purple-800 bg-purple-50 dark:bg-purple-900/30 p-3">
              <span className="text-xs font-medium text-purple-800 dark:text-purple-300">Reason for {statusLabel(pendingHoldStatusId)}:</span>
              <select
                value={holdReasonId}
                onChange={(e) => setHoldReasonId(e.target.value)}
                className="rounded-md border border-purple-300 dark:border-purple-700 px-2 py-1 text-xs"
              >
              ...
              <button
                ...
                className="rounded-md bg-purple-600 dark:bg-purple-500 px-2 py-1 text-xs font-medium text-white hover:bg-purple-700 dark:hover:bg-purple-600 disabled:opacity-50"
              >
                Confirm
              </button>
              <button
        ...
        <div className="rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5">
          <h2 className="mb-3 text-sm font-semibold text-slate-700 dark:text-slate-300">Subtasks</h2>
          <ul className="mb-3 space-y-1">
            {subtasks?.items?.map((s: any) => (
              <li key={s.id} className="flex items-center justify-between text-sm">
                <Link to={`/tasks/${s.id}`} className="text-slate-700 dark:text-slate-300 hover:text-brand-700 dark:hover:text-brand-300 hover:underline">
                  {s.title}
                </Link>
        ...
        <div className="rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5">
          ...
          <div className="space-y-3">
            {comments?.map((c: any) => (
              <div key={c.id} className="rounded-md bg-slate-50 dark:bg-slate-950 p-3 text-sm">
