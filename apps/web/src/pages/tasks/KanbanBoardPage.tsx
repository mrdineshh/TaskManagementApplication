        ...
        {sortedStatuses.map((status: any) => (
          ...
          >
            <div className="mb-2 flex items-center justify-between px-1">
              ...
              <span className="text-xs text-slate-400 dark:text-slate-500">{tasksByStatus.get(status.id)?.length ?? 0}</span>
            </div>
            <div className="space-y-2">
              {(tasksByStatus.get(status.id) ?? []).map((t: any) => (
                ...
                >
                  <Link to={`/tasks/${t.id}`} className="text-sm font-medium text-slate-800 dark:text-slate-200 hover:text-brand-700 dark:hover:text-brand-300">
                    {t.title}
                  </Link>
                  <div className="mt-2 flex items-center justify-between">
                    {t.priority && <Badge label={t.priority.label} color={t.priority.color} />}
                    {t.assignee && <span className="text-xs text-slate-400 dark:text-slate-500">{t.assignee.full_name}</span>}
                  </div>
                </div>
              ))}
            </div>
