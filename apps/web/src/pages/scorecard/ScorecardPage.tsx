  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5">
        <h2 className="text-sm font-semibold text-slate-700 dark:text-slate-300">My scorecard</h2>
        {mineLoading && <p className="mt-3 text-sm text-slate-400 dark:text-slate-500">Loading…</p>}
        {mine && (
          <>
            <div className="mt-4 flex items-baseline gap-3">
              <span className="text-4xl font-bold text-brand-600 dark:text-brand-400">{mine.overall_score}</span>
            ...
            <div className="mt-5 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
              {Object.entries(mine.sub_scores).map(([key, value]) => (
                <div key={key} className="rounded-md border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 p-3">
                  <p className="text-xs font-medium uppercase text-slate-400 dark:text-slate-500">{SUB_SCORE_LABELS[key] ?? key}</p>
                  <p className="mt-1 text-xl font-semibold text-slate-800 dark:text-slate-200">{value}</p>
                </div>
              ))}
            </div>
            <dl className="mt-5 grid grid-cols-2 gap-x-6 gap-y-1 text-xs text-slate-500 dark:text-slate-400 sm:grid-cols-3">
              <div>
                <dt className="inline text-slate-400 dark:text-slate-500">Completed: </dt>
                <dd className="inline font-medium text-slate-700 dark:text-slate-300">{mine.raw.completed_count}</dd>
              </div>
              <div>
                <dt className="inline text-slate-400 dark:text-slate-500">On time: </dt>
                <dd className="inline font-medium text-slate-700 dark:text-slate-300">{mine.raw.on_time_count}</dd>
              </div>
              <div>
                <dt className="inline text-slate-400 dark:text-slate-500">Overdue: </dt>
                <dd className="inline font-medium text-slate-700 dark:text-slate-300">{mine.raw.overdue_count}</dd>
              </div>
              <div>
                <dt className="inline text-slate-400 dark:text-slate-500">Over budget: </dt>
                <dd className="inline font-medium text-slate-700 dark:text-slate-300">{mine.raw.over_budget_count}</dd>
              </div>
              <div>
                <dt className="inline text-slate-400 dark:text-slate-500">Reworked: </dt>
                <dd className="inline font-medium text-slate-700 dark:text-slate-300">{mine.raw.reworked_count}</dd>
              </div>
              {mine.raw.avg_estimate_error_pct !== null && (
                <div>
                  <dt className="inline text-slate-400 dark:text-slate-500">Avg estimate error: </dt>
                  <dd className="inline font-medium text-slate-700 dark:text-slate-300">{mine.raw.avg_estimate_error_pct}%</dd>
                </div>
              )}
            </dl>
          </>
      ...
      <section className="rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
        ...
        <ol>
          {leaderboard?.map((entry) => (
            <li
              key={entry.user_id}
              className={`flex items-center justify-between border-b border-slate-100 dark:border-slate-800 px-5 py-3 text-sm last:border-0 ${
            ...
            >
              <span className="flex items-center gap-3">
                ...
                <span className="font-medium text-slate-800 dark:text-slate-200">{entry.full_name}</span>
              </span>
