...
export function OrgSettingsAdminPage() {
  ...
  return (
    <form onSubmit={handleSave} className="max-w-md space-y-3 rounded-lg border border-slate-200 bg-white p-4">
      <div>
        <label className="mb-1 block text-xs font-medium text-slate-500">Organization name</label>
      ...
      <div>
        ...
        <input value={timezone} onChange={(e) => setTimezone(e.target.value)} placeholder="Asia/Kolkata" className="w-full rounded-md border border-slate-300 px-3 py-1.5 text-sm" />
      </div>
