...
export function ScorecardWeightsAdminPage() {
  ...
  if (isLoading || !weights) return <p className="text-slate-400 dark:text-slate-500">Loading…</p>;
        ...
        {(Object.keys(weights) as (keyof ScorecardWeights)[]).map((key) => (
          <label key={key} className="flex items-center justify-between gap-4 text-sm">
            ...
            <input
              ...
              className="w-24 rounded-md border border-slate-300 dark:border-slate-700 px-2 py-1 text-right"
            />
          </label>
        ))}
        <div className={`flex justify-between border-t border-slate-100 dark:border-slate-800 pt-3 text-sm font-medium ${Math.abs(total - 1) > 0.001 ? 'text-red-600 dark:text-red-400' : 'text-slate-700 dark:text-slate-300'}`}>
          <span>Total</span>
